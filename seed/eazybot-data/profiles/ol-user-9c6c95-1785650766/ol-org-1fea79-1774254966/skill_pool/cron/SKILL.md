---
name: cron
description: 仅在需要未来定时执行或周期执行任务时，使用本 skill。使用 eazybot cron list/create/get/state/pause/resume/delete/run 管理任务，并始终显式传入 --agent-id。
metadata:
...
```bash
# 列出任务
eazybot cron list --agent-id <agent_id>

# 查看任务详情
eazybot cron get <job_id> --agent-id <agent_id>

# 查看任务状态
eazybot cron state <job_id> --agent-id <agent_id>

# 创建任务
eazybot cron create --agent-id <agent_id> ...

# 删除任务
eazybot cron delete <job_id> --agent-id <agent_id>

# 暂停 / 恢复任务
eazybot cron pause <job_id> --agent-id <agent_id>
eazybot cron resume <job_id> --agent-id <agent_id>

# 立即执行一次已有任务
eazybot cron run <job_id> --agent-id <agent_id>
```
...
```bash
eazybot cron create --agent-id <agent_id> -f job_spec.json
```
...
- 缺少参数时，先问用户再创建
- 修改/暂停/删除前，先 `eazybot cron list --agent-id <agent_id>`
- 排查问题时，用 `eazybot cron state <job_id> --agent-id <agent_id>`
...
eazybot cron -h
eazybot cron list -h
eazybot cron create -h
eazybot cron get -h
eazybot cron state -h
eazybot cron pause -h
eazybot cron resume -h
eazybot cron delete -h
eazybot cron run -h
```
