...
description: 当需要其他 agent 的专长、上下文或协作支持，或用户明确要求调用其他 agent 时，使用本 skill。先查询可用 agents，再用 eazybot agents chat 进行双向沟通。
metadata:
...
```bash
eazybot agents chat --background \
  --from-agent <your_agent> \
  --to-agent <target_agent> \
  --text "[Agent <your_agent> requesting] ..."
```
...
```bash
eazybot agents chat --background --task-id <task_id>
```
...
| 任务类型 | 使用模式 | 命令 |
|---------|---------|------|
| 简单快速查询 | 实时模式 | `eazybot agents chat` |
| 复杂任务（数据分析、批量处理等） | 后台模式 | `eazybot agents chat --background` |
...
2. eazybot agents list
3. eazybot agents chat 发起对话
4. 从输出中记录 [SESSION: ...]
...
2. eazybot agents list
3. eazybot agents chat --background 提交任务
4. 从输出中记录 [TASK_ID: ...]
5. 继续处理其他工作
...
Check status with:
  eazybot agents chat --background --task-id 20802ea3-...
```
...
# 方式 2：如果必须等待，使用合理间隔
sleep 30 && eazybot agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
# 3. 在适当时机查询结果
# （比如处理完当前任务后，或用户询问进度时）
eazybot agents chat --background --task-id <id>
```

**方式 2：定时轮询**（如果必须等待）
```bash
# 递增间隔，先快后慢
sleep 10 && eazybot agents chat --background --task-id <id>
sleep 20 && eazybot agents chat --background --task-id <id>
sleep 30 && eazybot agents chat --background --task-id <id>
```
...
```bash
eazybot agents -h
eazybot agents list -h
eazybot agents chat -h
```
