...
# 读取 Excel：默认只读第一个 sheet；大文件先 peek 再决定读多少
df = pd.read_excel('file.xlsx')                              # 单表默认
df = pd.read_excel('file.xlsx', nrows=1000)                  # 大文件：先读前 1000 行探路
df = pd.read_excel('file.xlsx', usecols=['A', 'C', 'E'])     # 只要需要的列

# ⚠️ 禁止用 sheet_name=None 一次读全部 sheet——会把所有数据全量载入内存
# 多表场景逐表读取（先用 ExcelFile 探 sheet 名，不载入数据）：
# sheet_names = pd.ExcelFile('file.xlsx').sheet_names
# df2 = pd.read_excel('file.xlsx', sheet_name=sheet_names[1], nrows=1000)

# 分析数据（先 peek 摘要，输出写文件，不要 print 全表）
df.head()      # 前几行
df.info()      # 列信息与类型
df.describe()  # 数值统计
...
### 用 openpyxl 分析现有文件（大文件走只读模式）

需要精确控制单元格/公式/格式时用 openpyxl。**大文件分析务必用 `read_only=True` 流式读取**，它逐行解析、用完即弃，不会把整表 DOM 载入内存：

```python
from openpyxl import load_workbook

# read_only=True 流式读取 + data_only=True 取计算后的数值
wb = load_workbook('file.xlsx', read_only=True, data_only=True)
ws = wb['SheetName']  # 或 wb.active

# 先 peek 维度与表头，不要一上来就全量遍历
...
for row in ws.iter_rows(values_only=True):
    ...  # row 是 (A列值, B列值, ...) 元组；统计/过滤结果写文件，别 print 全表

wb.close()  # 只读模式用完必须关闭，释放文件句柄
```
...
```python
# 编辑现有文件：用可写模式（默认）。不要加 read_only=True——
# insert_rows / delete_cols / create_sheet 等结构修改需要可写 DOM，只读模式会抛错
from openpyxl import load_workbook
...
**仅当本次写入或修改了公式时才运行 `recalc.py`；纯读取/分析、无公式写入需求时不要跑它**（它会调 LibreOffice 全量重算，耗时且吃资源）。
...
- 大文件先用 `nrows` 限制行数探路：`pd.read_excel('file.xlsx', nrows=1000)`
- 正确处理日期：`pd.read_excel('file.xlsx', parse_dates=['date_column'])`
- ⚠️ 禁止用 `sheet_name=None` 一次读全部 sheet（全量载入内存）；需要多表时逐表 `sheet_name=<单表名>` 读
